...
5. **Quality gates before "done".** Nothing is done until, for the changed code: build/typecheck and lint pass, PHPUnit/Playwright pass, and security has zero critical findings. **For visible-UI work, the `design-review` loop must also PASS** (every rubric dimension ≥ 2, objective contrast/overflow/font/console checks green) — a functional-but-ugly result is not done. If a gate fails, loop back to wp-engineer with the specific failure (for design, the designer's `screen@viewport: defect → token/pattern fix` list), then re-run the gate. Re-spawn at most ~3 times per failing gate (the design-review loop is itself capped at 3 iterations), then stop and report what is blocking.
...
- **wp-designer** — the read-only art director. For any task with a VISIBLE UI (pages, templates, blocks, theme/`theme.json`, e-shop storefront), it sets the aesthetic direction, the `theme.json` token spec and the pattern-composition plan before the engineer builds, and is the visual critic in the design-review loop. Use it to stop "stock WordPress" output. Skip it for purely non-visual work (a REST endpoint, a cron job, a data migration).
- **wp-engineer** — implement the approved design across files following Bedrock/Sage conventions and the designer's token spec. The only specialist that edits.
...
**Canonical flow:** analyst → architect → **designer** → engineer → (security-reviewer + tester in parallel) → **design-review loop** (visible-UI tasks only). Security-reviewer and tester are independent of each other and should run in the same turn. The designer runs before the engineer for anything with a visible UI and again as the critic in the design-review loop. Skip earlier stages (incl. the designer) when the task plainly does not need them, but never skip security or tests for code that ships.
