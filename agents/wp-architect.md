...
- **Client self-editing:** editable regions use `templateLock`/`contentOnly`; client roles are restricted (see `mu-plugins/loamkit-roles.php`). Respect this when designing editor-facing blocks.
- **Design system (visible UI):** the project ships an opinionated design system — `theme.json` design tokens (the 10-slug palette, fluid type scale, spacing rhythm, self-hosted `heading`/`body` fonts), `/styles` section-style colorways, and a `/patterns` block-pattern library (see `reference/design-system.md`). For anything visual, design **with** these: plan pages as a composition of existing patterns + section styles, and defer the aesthetic direction + the exact token spec to **wp-designer** (it runs before the engineer). Never plan "raw core blocks on a white page", and never plan hardcoded colors/sizes — every value is a `theme.json` preset token. Flag where a NEW pattern is genuinely needed.
