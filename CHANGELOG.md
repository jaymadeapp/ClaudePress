...
Public launch. The kit is now **Loamkit** (formerly ClaudePress) — renamed to drop
"Claude" from the product name (trademark hygiene) and match the repo,
`github.com/jaymadeapp/Loamkit`.

### Changed

- **Rebrand: ClaudePress → Loamkit, everywhere.** Plugin + marketplace name `loamkit`
  (`/plugin marketplace add jaymadeapp/Loamkit` → `/plugin install loamkit@loamkit`),
  skills `/loamkit:wp-setup` / `/loamkit:intake` / `/loamkit:deploy-staging`, mu-plugin
  templates `loamkit-{roles,design,woo}.php`, the seeder command `wp loamkit seed`, the
  MCP user/role `loamkit-mcp` / `loamkit_mcp`, the CSS component prefix `.cp-*` → `.lk-*`
  and the deploy env vars `CP_STAGING_*` → `LK_STAGING_*`. **Breaking** for projects
  generated by ≤ 0.6.x: names only — regenerate, or rename the mu-plugins/classes/env
  vars by hand. The guard self-test suite passes unchanged (48/48) after the rename.
- **README overhaul for launch.** English-only (the Czech quick-start block is gone),
  a hero with badges and live screenshots (`docs/assets/`), the design-system content
  restructured into its own section, and the documented PHP floor corrected to
  **PHP 8.4+** (Sage 11 / Acorn 6 require ≥ 8.4.1 — the README previously said 8.3+).

### Added

- **Launch screenshots** in `docs/assets/` — homepage and WooCommerce shop/product
  pages (desktop + mobile), captured with Playwright from a real generated Terra
  demo site (vanilla-WP compose demo assembled from the bundled templates).
...
- **CTA band ("Bring a little more calm *home*").** The accent word, the button and the
  ...
  - Button → an **inverse pill**: an `on-primary` fill with `primary`-coloured text, set via a
    `.lk-cta-pill` class (WordPress regenerates a core/button's inline style from block attrs, so
...
- **PDP sticky buy box overlapped the content below it on scroll** (the reported bug). The
  ...
  **not** clip a sticky grid item to its own grid row, so the buy box rode down over the
  ...
  the gallery + buy box in `.lk-product-top` (markup-only, via the
...
- **Store "missing margin".** WooCommerce's classic templates (shop, single product, account,
  ...
  constrained. The content wrapper is now re-pointed to a `.lk-store` container with the **same
...
- Inset-spine unification (one `spacing--40` inset across root, chrome, `.lk-store` and every
...
- **Component utilities + motion** — `.lk-blob`/`.lk-blob-img` (organic shapes), `.lk-frame`
  (framed image with depth), `.lk-reveal` scroll-reveal (IntersectionObserver in the design
